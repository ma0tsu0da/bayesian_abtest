...
st.write("""
         パラメータ（ここでは開封率やクリック率）の事前分布と、そのパラメータのもとで生成される
         実現値（ここでは対象者の開封・クリック行動）をもとに、A・Bそれぞれの「事後分布」を得る。
         ...
         その分布を比較することで、効果を検証する。
         上記プロットは、Aの開封率（クリック率）とBの開封率（クリック率）の差の確率分布であり、
         確率分布の正の領域の面積は、BがAより大きくなる確率となる。
         """)
